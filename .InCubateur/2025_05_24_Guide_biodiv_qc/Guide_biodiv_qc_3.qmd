...
amniotes_traits = read.csv("data/partie_3/amniote/Amniote_Database_Aug_2015.csv", na.strings = -999)
...
```{r}
out_img_folder = "output/images"
inat_path = file.path(out_img_folder, "iNat")
dir.create(
  path = inat_path, 
  showWarnings = FALSE,
  recursive = TRUE)
...
for (sp_check_i in 1:nrow(sp_list)) {
  ...
  fichiers_local = list.files(path = inat_path,
                              pattern = ".jpg", 
...
Toute cette réflexion sur le 'dépliant format papier' fait réfléchir sur le développement d'un 'dépliant format numérique'. L'idée de ce projet est de montrer comment penser qu'un dépliant est possible avec les analyses proposées. Toutes les étapes de ce projet peuvent être généralisées pour les MRC du Québec. À ce moment, il est possible d'en faire un tableau de bord (<a href='https://shiny.posit.co' target='_blank'>Shiny App</a>?). Ainsi, nous pourrions imaginer une sorte d'application ou portail où il est possible de choisir une région et d'avoir toutes les informations du dépliant généré automatiquement par cette procédure. Pour le moment, cela est un exercice pour une autre fois...
