---
title: "Guide d'identification de biodiversité du Québec - Partie 2 : Les données de biodiversité"
...
# csl: ../evolution.csl
date: "2025-05-24"
categories: ["biodiversité", "analyse", "tutoriel"]
...
<!-- [billet sur la cartographie](Guide_biodiv_qc_1.qmd) -->

## plan

1.  C'est quoi des données de biodiversités: Types de données de biodiv (occurences)

-   qu'est-ce qu'il faut savoir sur les données de biodiversité
-   Biais potentiel par le type de données

2.  Qui participe à bonifier les données de biodiversité?

-   Certains scientifiques, musées, gouvernements, entreprises ou citoyen participent à la collecte de données de biodiversité. Oui chère personne lectrice. Vous pouvez aider à [inlcude un guide pour iNaturalist](https://www.youtube.com/watch?v=VamIMCxDyOA&list=PLY-KTbpNXtBE5wJW2hvnNb1aMolnMtypE)
-   Vous êtes plus oiseaux? eBird
-   Vous vous intéressez à la biodiversité en général, mais vous avez besoin d'aide au début pour identifier? iNaturalist.
-   Vous devez faire de l'échantillonnage terrain (comme des points d'écoutes ou des transects)? Observation.org ou NatureCounts (si vous participez à un protocole)
-   Vous participez à des activités de support de Savoirs écologiques traditionnels? GeoKeeper. (voir aussi : https://www.gbif.org/news/1Ke3Gk2USgdIW5OgDlBIKY/open-data-for-people-and-purpose-gbif-establishes-task-group-on-indigenous-data-governance)
-   Pourquoi autant d'applications différentes? Elles ont des objectifs différents, des protocoles d'échantionnallage différents, un focus sur des groupes taxonomiques ([eBird](https://ebird.org/home), [Pl\@ntNet](https://plantnet.org), [eButterfly](https://espacepourlavie.ca/ebutterfly)). Pour certaines, sont des réseaux sociaux qui ont des communautés qui se supportent différement. Par exemple, les ornithologues cherches des informations différentes que les botanistes. 3.1 Où trouver des données?
-   Beaucoup des plateformes citées plus haut partagent la totalité ou une partie de leurs données. Par exemple, iNaturalist partage seulement les observations de « qualité recherche », c'est-à-dire dont au moins deux personnes ont validées. 3.2 Comment extraire et manipulation sans HPC! (OLAP)
-   Presque chaque site de science citoyenne permet de télécharger des données.
-   Par contre, **LA** référence est [GBIF](https://www.gbif.org).\
-   Tutoriel de téléchargement

4.  Portrait de biodiversité\

-   Portrait comparatif : Visualisation (carte de lumière, carte H3, histogramme taxa?)
-   Exploration des données du Québec

5.  Sélection d'espèces pour le guide selon fréquence d'observation par groupe taxonomique

-   Prendre les groupes taxonomiques qui sont intéressants seulement.
...
DBI::dbExecute(
  conn = con, 
  statement = sprintf("CREATE VIEW %s AS SELECT * FROM read_csv('%s')", 
                      nom_vue, 
                      csv))
...
write.csv(
  x = inatdb_animaux, 
  file = file.path('data/gbif_data', 
                   "inat_research_grade_obs_animalia.csv")
  )

```

Performance : Ne prend que quelques secondes sur un ordinateur portable avec les données sur un disque dur (ce qui n'est vraiment pas le plus rapide!). Il faudrait tester avec un 'disque' NVMe SSD qui est beaucoup plus rapide.\
